...
import { parseEditoListing, scrapeEdito, type EditoRawEvent } from './edito.js';

/**
 * Muzeum Plakatu w Wilanowie (postermuseum.pl) — GOI-57.
 *
 * The museum has two programmes on two differently-shaped pages, and the issue
 * asks for both, differently:
 *
 *   /wydarzenia/kalendarz-wydarzen/  → date **and** time
 *   /wystawy/                        → dates only
 *
 * The calendar is the same edito CMS module MNW and Królikarnia run — the site
 * even declares `<meta name="generator" content="edito">` — so it needs no
 * parser of its own; `parseEditoListing` reads it as-is.
 *
 * The exhibitions page is the new part. It carries no dated rows at all, only
 * a run written out in Polish prose:
 *
 *   <div class="news">
 *     <h1 class="title"><a href="/wystawy/…,89.html">Plakat polski. Kolekcja</a></h1>
 *     <div class="desc">20 czerwca - 13 września 2026</div>
 *
 * That run is expanded to one all-day row per day it covers, clipped to the
 * scrape window. Per-day rows are how exhibitions already reach the app — the
 * MNW calendar emits them that way and `dedupeAllDay` collapses the run back
 * into a single card (GOI-53). Emitting one row at the opening date instead
 * would read as "started in June", and a listing would drop it the next
...
export function expandRun(
  ...
  for (let day = first; day <= last; day = nextDay(day)) {
    ...
    out.push({
      ...
      source_id: run.sourceId ? `postermuseum:wystawa:${run.sourceId}:${day}` : null,
    });
  }
  return out;
}
...
export function parsePosterMuseum(
  ...
  if (/module-exhibitions/.test(html)) {
    const horizon = ymdInTz(new Date(today.getTime() + 60 * 86_400_000), timeZone);
    return parseExhibitionsListing(html, `${ORIGIN}/wystawy/obecne.html`, todayYmd).flatMap((r) =>
      expandRun(r, todayYmd, horizon, timeZone),
    );
  }
  return parseEditoListing(html, `${ORIGIN}/wydarzenia/kalendarz-wydarzen/`, timeZone);
}

/**
 * Both programmes in one pass: the edito calendar for timed events, the
 * exhibition pages for the all-day runs.
 *
 ...
 */
export async function scrapePosterMuseum(args: {
  baseUrl: string;
  today: Date;
  windowDays: number;
  timezone?: string;
  ...
  for (const path of EXHIBITION_PATHS) {
    ...
    try {
      ...
      for (const run of parseExhibitionsListing(html, url, fromDay)) {
        exhibitions.push(...expandRun(run, fromDay, toDay, tz));
      }
    } catch (e) {
      console.warn(`[postermuseum] failed to fetch ${url}: ${e instanceof Error ? e.message : e}`);
    }
  }
